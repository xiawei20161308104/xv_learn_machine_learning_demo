...
LastEditors: xiawei
LastEditTime: 2022-12-27 12:59:33
Description: 有前景背景无未知区域,connectedComponentsWithStats
锐化-灰度化-高斯滤波-二值化-开闭运算-得到未知-connectedComponentsWithStats-makers+1-未知置0-wateshed
'''
# 距离变换与分水岭算法
...
def Watershed():
    ...
    close = cv.morphologyEx(
        binary, cv.MORPH_CLOSE, (3, 3), iterations=1)
    ...
    unknown = cv.subtract(close, sure_fg)
    """
    etval : 返回值是连通区域的数量。
